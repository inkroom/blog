...
> 
> 所以我是去[jellyfin](https://github.com/jellyfin/jellyfin/blob/master/.devcontainer/install-ffmpeg.sh)中重新安装的ffmpeg


结果是成功了一半，有些字正常，有些字不正常。

再使用了[FontViewOk](https://www.softwareok.com/?Download=FontViewOK)比对字体渲染后，确定是字体不支持这些字。一通查找下知道了字体子集化这个概念，就是只打包要渲染的文字，这样可以有效减小字体文件大小，特别适合视频字幕这种文字已知的场景
