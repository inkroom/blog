...
if (!(hexo.config.archive && hexo.config.archive.enabled === false)) {
    ...
    hexo.extend.generator.register('archive', function(locals) {
        ...
        const allPosts = locals.posts.filter(s=>s.categories.data.findIndex(m=>m.name =='private')==-1).sort(config.archive_generator.order_by || '-date');
        ...
        allPosts.forEach(post => {
            if(post.title=='dream'){
                console.log('post',post.categories.data.findIndex(m=>m.name=='private'),post.categories.data)
            }
